...
- Routes (each is a Fastify plugin):
  ...
  - [auth.ts](server/src/routes/auth.ts) — bcrypt + JWT register/login + `/api/me` (returns `ownedLocationIds` and `curatedVillages`).
  ...
  - [admin-users.ts](server/src/routes/admin-users.ts) — admin: search users, change role, grant/revoke object ownership, grant/revoke **village curatorship**. The list endpoint exposes `ownedLocations[]` and `curatedVillages[]` per row.
...
- **Auth gate:** [RequireAuth.tsx](web/src/admin/RequireAuth.tsx) checks JWT presence + the decoded `role` against an optional `role` prop (currently `'admin' | 'business' | 'curator'`). JWT is in `localStorage` under `gnd.token`. `CurrentUser` from `/api/me` carries `ownedLocationIds: number[]` AND `curatedVillages: string[]`.
