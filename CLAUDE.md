...
- Routes (each is a Fastify plugin):
  ...
  - [villages.ts](server/src/routes/villages.ts) — public `GET /api/villages` returning facts + curators list per village (no PII — only `id` and `displayName`). Drives `/naselja` and `/naseljageo`.
...
- Routes (all under the `App` shell so nav + context are always present):
  ...
  - `/naseljageo` — **lazy-loaded** Leaflet variant with village seat markers (gold for Žabari, navy elsewhere) and a placeholder GeoJSON layer (FeatureCollection with empty `features` for now; real RZS/OSM polygons TBD). Reachable only by URL — not in the top nav.
